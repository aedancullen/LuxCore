...
SwingEnvironmentCamera::SwingEnvironmentCamera(const Point &o, const Point &t, const Vector &u, const float *sw) :
		Camera(SWINGENVIRONMENT), lensRadius(0.f), focalDistance(10.f), autoFocus(false),
...
Properties SwingEnvironmentCamera::ToProperties() const {
	Properties props = Camera::ToProperties();

	props.Set(Property("scene.camera.type")("swingenvironment"));
...
void SwingEnvironmentCamera::GenerateRay(const float filmX, const float filmY, Ray *ray,
...
bool SwingEnvironmentCamera::GetSamplePosition(Ray *ray, float *x, float *y) const {	
...
bool SwingEnvironmentCamera::SampleLens(const float time, const float u1, const float u2, Point *lensp) const {
...
float SwingEnvironmentCamera::GetPDF(const Vector &eyeDir, const float filmX, const float filmY) const {
...
void SwingEnvironmentCamera::InitCameraTransforms(CameraTransforms *trans) {
...
void SwingEnvironmentCamera::InitPixelArea() {
